...
public class NFC extends Activity {
    ...
    private void initial() {
        ...
        if (mNfcHandler.isNFCAdapterExist() && mNfcHandler.isSupportsNFC()) {
            ...
            Toast.makeText(this, mToolKit.getStringResource(R.string.nfc_not_support),
...
//			backResult(false);
    ...
    private void getTestArguments() {
        ...
        Log.i("W", "nfc test mode= " + mTestMode);
    }
    ...
    private void enableNdefExchangeMode() {
        ...
        mNfcHandler.enableForegroundNdefPush(NFC.this, getNoteAsNdef());
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (mTestMode == 0) {
            ...
            if (uid != null) {
                (findViewById(R.id.nfc_read_section)).setVisibility(View.VISIBLE);
                (findViewById(R.id.nfc_read_title)).setVisibility(View.INVISIBLE);
                tvNFCReadContent.setText(uid);
//                tvNFCReadContent.setVisibility(View.VISIBLE);
            }
            backResult(true);
        } else {
            ...
            // NDEF exchange mode
            if (!isWriteMode
                    && (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction()) || NfcAdapter.ACTION_TECH_DISCOVERED
                ...
                if (msgs != null) {
                    ...
                    if ((mTestMode & TEST_MODE_WRITE_AND_READ) != 0) {
                        ...
                        // the app will exit after wait 2 seconds
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if (mReadContent != null) {
                                    handler.obtainMessage(MSG_EXIT).sendToTarget();
                                } else {
                                    handler.obtainMessage(MSG_TIMEOUT).sendToTarget();
                                }
                            }
                        }).start();
                        // if (mReadContent != null) {
    ...
    private boolean writeTag(NdefMessage message, Tag tag) {
        ...
        try {
            ...
            if (ndef != null) {
                ndef.connect();
                if (!ndef.isWritable()) {
                    tvNFCWriteContent.setText(mToolKit
                            .getStringResource(R.string.nfc_write_error_read_only));
                } else {
                    if (ndef.getMaxSize() < size) {
                        tvNFCWriteContent.setText(String.format(
                                mToolKit.getStringResource(R.string.nfc_write_error_over_capacity),
                ...
                if (format != null) {
                    try {
                        ...
                        e.printStackTrace();
                        tvNFCWriteContent.setText(mToolKit
                                .getStringResource(R.string.nfc_write_error_format_fail));
                    }
                } else {
                    tvNFCWriteContent.setText(mToolKit
                            .getStringResource(R.string.nfc_write_error_not_support_ndef));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            isWriteFail = true;
            tvNFCWriteContent.setText(mToolKit
                    .getStringResource(R.string.nfc_write_error_write_fail));
        }
        return false;
    }
    ...
    private void backResult(boolean finish) {
        Log.i("W", "NFC test result: " + finish);
        try {
